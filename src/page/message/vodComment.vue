 ...
 * @Last Modified time: 2020-05-19 10:37:30
 */
  ...
  <div>
    ...
    <div style="margin-bottom: 16px;">
      ...
      <a-button
        ...
        :disabled="unreadCount.vodCommentCount===0"
      ...
      <template v-slot:content="text, record">
        <a-tooltip placement="right" :title="text">
          <div class="text-overflow" @click="read(record)">
            <a :href="`${VOD.WEB_HREF}vodComment/detail?vodId=${record.vodComment.vod.id}&id=${record.vodComment.id}&page=1&jwt=${token}`" target="_blank">{{ text }}</a>
          </div>
        </a-tooltip>
      </template>
      <template v-slot:vod="text, record">
        <a-tooltip placement="right" :title="text">
          <div class="text-overflow">
            <a :href="`${VOD.WEB_HREF}detail?id=${record.vodComment.vod.id}&jwt=${token}`" target="_blank">{{ text }}</a>
          </div>
        </a-tooltip>
      </template>
      <template v-slot:user="text, record">
        {{ text || '用户' + record.vodComment.user.id }}
      </template>
...
import { readVodCommentAt, deleteVodCommentAt } from '@/api'
...
export default {
  data () {
    return {
      ...
      columns: [
        ...
        {
          title: '评论',
          dataIndex: 'vodComment.content',
          key: 'vodComment.content',
          scopedSlots: { customRender: 'content' },
          width: 300
        },
        {
          title: '评论者',
          dataIndex: 'vodComment.user.name',
          key: 'vodComment.user.name',
          scopedSlots: { customRender: 'user' },
          width: 200
        },
        {
          title: '影片',
          dataIndex: 'vodComment.vod.name',
          key: 'vod.name',
          scopedSlots: { customRender: 'vod' },
          width: 200
        },
        {
          title: '时间',
  ...
  computed: {
    ...mapState({
      list: store => store.message.vodCommentAtList,
  ...
  methods: {
    ...mapMutations('message', {
      setListItem: 'setVodCommentAtListItem',
      setUnreadCount: 'setUnreadCount'
    }),
    ...mapActions('message', {
      getList: 'getVodCommentAtList',
    ...
    read (record) {
      ...
      readVodCommentAt([ record.id ]).finally(() => {
        this.setListItem({
          id: record.id,
          read: 2,
          readLoading: false
        ...
        this.setUnreadCount({
          ...this.unreadCount,
          vodCommentCount: this.unreadCount.vodCommentCount - 1
        })
      })
    },
    readAll () {
      this.readAllLoading = true
      readVodCommentAt().then(() => {
        this.setUnreadCount({
          ...this.unreadCount,
          vodCommentCount: 0
        })
    ...
    handleDelete (record) {
      ...
      deleteVodCommentAt({ ids: [ record.id ] }).then((res) => {
        if (record.read === 1) {
          this.setUnreadCount({
            ...this.unreadCount,
            vodCommentCount: this.unreadCount.vodCommentCount - 1
    ...
    handleDeleteBatch () {
      ...
      deleteVodCommentAt({ ids }).then((res) => {
        this.getUnreadCount()
        this.deleteThen(res)
      }).finally(() => {
