/*
 * @Author: lin.zhenhui
 * @Date: 2020-05-13 15:05:48
 * @Last Modified by: lin.zhenhui
 * @Last Modified time: 2020-05-19 10:37:13
 */
  ...
  <div>
    ...
    <div style="margin-bottom: 16px;">
      ...
      <a-button
        ...
        :disabled="unreadCount.vodCollectionCommentCount===0"
      ...
      <template v-slot:content="text, record">
        <a-tooltip placement="right" :title="text">
          <div class="text-overflow" @click="read(record)">
            <a :href="`${VOD.WEB_HREF}collection/detail?id=${record.vodCollectionComment.vodCollectionID}&page=1&cPage=1&jwt=${token}`" target="_blank">{{ text }}</a>
          </div>
        </a-tooltip>
      </template>
      <template v-slot:user="text, record">
        {{ text || '用户' + record.vodCollectionComment.user.id }}
      </template>
...
import { readVodCollectionCommentAt, deleteVodCollectionCommentAt } from '@/api'
...
export default {
  data () {
    return {
      ...
      columns: [
        ...
        {
          title: '评论',
          dataIndex: 'vodCollectionComment.content',
          key: 'vodCollectionComment.content',
          scopedSlots: { customRender: 'content' },
          width: 300
        },
        {
          title: '评论者',
          dataIndex: 'vodCollectionComment.user.name',
          key: 'vodCollectionComment.user.name',
  ...
  computed: {
    ...mapState({
      list: store => store.message.vodCollectionCommentAtList,
  ...
  methods: {
    ...mapMutations('message', {
      setListItem: 'setVodCollectionCommentAtListItem',
      setUnreadCount: 'setUnreadCount'
    }),
    ...mapActions('message', {
      getList: 'getVodCollectionCommentAtList',
    ...
    read (record) {
      ...
      readVodCollectionCommentAt([ record.id ]).finally(() => {
        this.setListItem({
          id: record.id,
          read: 2,
          readLoading: false
        ...
        this.setUnreadCount({
          ...this.unreadCount,
          vodCollectionCommentCount: this.unreadCount.vodCollectionCommentCount - 1
        })
      })
    },
    readAll () {
      this.readAllLoading = true
      readVodCollectionCommentAt().then(() => {
        this.setUnreadCount({
          ...this.unreadCount,
    ...
    handleDelete (record) {
      ...
      deleteVodCollectionCommentAt({ ids: [ record.id ] }).then((res) => {
        if (record.read === 1) {
          this.setUnreadCount({
            ...this.unreadCount,
            vodCollectionCommentCount: this.unreadCount.vodCollectionCommentCount - 1
    ...
    handleDeleteBatch () {
      ...
      deleteVodCollectionCommentAt({ ids }).then((res) => {
        this.getUnreadCount()
        this.deleteThen(res)
      }).finally(() => {
